...
$auteur = $db->query("SELECT auteur FROM commentaires WHERE id = '$gt_id'");
$aut_com = $auteur->fetch();

if($_SESSION['pseudo'] != $aut_com['auteur']){
...
$res = $db->query("SELECT  * FROM commentaires WHERE id = '$id'");
$id_com = $res->fetch();
...
if(isset($_POST['valider'])){
	if(isset($_SESSION['pseudo']) && !empty($_SESSION['pseudo']) && isset($_POST['contenu']) && !empty($_POST['contenu']) ){
		...
		$delart = $db->prepare("UPDATE commentaires SET contenu = ? WHERE id = ?");
		$delart->execute(array($contenuS, $id_com['id']));
...
<form action="" method="post">
	Contenu: <textarea name="contenu" ><?php echo $id_com['contenu']; ?></textarea><br><br>
